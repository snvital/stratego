...
/**
 * Apply change of pieces to game
 * Emits an "update" event on success or an "error" event on failure
 */
var change = function(data) {
  ...
  var result = game.change(data.move);
  console.log(result);
  if (!result) {
    console.log('ERROR: Failed to Apply Move', debugInfo);
    this.emit('error', {message: "Invalid change, please try again"});
    return;
  }
...
var ready = function(gameID) {
  ...
  // ready game
  var result = game.ready(sess);
  if (!result) {
    console.log('ERROR: Failed to Ready', debugInfo);
    this.emit('error', {message: "Failed to get the game ready"});
    return;
  }
  ...
  console.log(gameID+' '+sess.playerName+': is ready');
};
